import './trix-aligment-elements.js';
...
document.addEventListener("trix-initialize", function (event) {
    ...
    const groups = toolbar.querySelectorAll(".trix-button-group");
    const alignGroup = document.createElement("span");

    alignGroup.className = "trix-button-group";

    alignGroup.innerHTML = `
        <button type="button" data-trix-attribute="alignLeft" title="Align Left" class="trix-button trix-button--icon trix-button--icon-align-left" tabindex="-1">Align Left</button>
        <button type="button" data-trix-attribute="alignCenter" title="Align Center" class="trix-button trix-button--icon trix-button--icon-align-center" tabindex="-1">Align Center</button>
        <button type="button" data-trix-attribute="alignRight" title="Align Right" class="trix-button trix-button--icon trix-button--icon-align-right" tabindex="-1">Align Right</button>
    `;

    // insert buttons group before file attachment button
    groups[2].before(alignGroup);

    // add font formatting group
    ...
    event.target.addEventListener("trix-attachment-add", function (attachmentEvent) {
        if (!attachmentEvent.attachment.file) return;
        ...
        uploadTrixImage(attachmentEvent.attachment);
    });
});
...
function uploadTrixImage(attachment)
{
    ...
    axios.post('/admin/trix-upload', formData, {
        headers: {
