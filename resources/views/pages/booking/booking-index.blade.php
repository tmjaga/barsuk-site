                ...
                <div>
                    ...
                    <ul class="space-y-1 mb-4">
                        <template x-for="s in selectedServices">
                            <li x-text="s.title_localized + ' — ' + s.price + ' &euro;'"></li>
                        </template>
                    </ul>

                    <div class="space-y-1 mb-4 text-sm">
                        <div>{{ __('Total Duration') }}: <strong x-text="totalDuration"></strong></div>
                        <div>{{ __('Total Price') }}: <strong x-text="totalPrice + ' &euro;'"></strong></div>

                        <div>{{ __('At Date') }}: <strong x-text="selectedDate"></strong></div>
                        <div>{{ __('At Time') }}: <strong x-text="selectedTime"></strong></div>
                    </div>

                    <div class="space-y-2">
                        <input x-model="formData.names" placeholder="{{ __('Name') }}" class="contact-field placeholder-gray-500 p-2 mb-2 w-full border rounded">
                        <p x-show="$v.formData.names.$invalid && $v.$touch" class="text-danger text-sm">{{ __('Please enter a valid Names (Min 8 characters)') }}</p>

                        <input x-model="formData.email" placeholder="{{ __('Email') }}" class="contact-field mb-2 w-full border p-2 rounded">
                        <p x-show="$v.formData.email.$invalid && $v.$touch" class="text-danger text-sm">{{ __('Please enter a valid Email') }}</p>

                        <input x-model="formData.phone" placeholder="Phone" class="contact-field w-full border p-2 rounded">
                        <p x-show="$v.formData.phone.$invalid && $v.$touch" class="text-danger text-sm">{{ __('Please enter a valid Phone Number') }}</p>
                    </div>

                    <div class="flex gap-3 mt-4">
                        <button @click="prev()" class="px-4 py-2 border rounded">{{ __('Back') }}</button>
            ...
            <template x-if="!loading && step === 4">
                <div class="text-center">
                    ...
                    <div x-show="!errors?.order" class="relative flex items-center justify-center z-1 mb-7">
                        <svg class="fill-success-50" width="90" height="90" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span class="absolute -translate-x-1/2 -translate-y-1/2 left-1/2 top-1/2">
                            <svg class="fill-success-600" width="38" height="38" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <!-- error icon-->
                    <div x-show="errors?.order" class="relative flex items-center justify-center z-1 mb-7">
                        <svg class="fill-error-50" width="90" height="90" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M34.364 6.85053C38.6205 -2.28351 51.3795 -2.28351 55.636 6.85053C58.0129 11.951 63.5594 14.6722 68.9556 13.3853C78.6192 11.0807 86.5743 21.2433 82.2185 30.3287C79.7862 35.402 81.1561 41.5165 85.5082 45.0122C93.3019 51.2725 90.4628 63.9451 80.7747 66.1403C75.3648 67.3661 71.5265 72.2695 71.5572 77.9156C71.6123 88.0265 60.1169 93.6664 52.3918 87.3184C48.0781 83.7737 41.9219 83.7737 37.6082 87.3184C29.8831 93.6664 18.3877 88.0266 18.4428 77.9156C18.4735 72.2695 14.6352 67.3661 9.22531 66.1403C-0.462787 63.9451 -3.30193 51.2725 4.49185 45.0122C8.84391 41.5165 10.2138 35.402 7.78151 30.3287C3.42572 21.2433 11.3808 11.0807 21.0444 13.3853C26.4406 14.6722 31.9871 11.951 34.364 6.85053Z" fill="" fill-opacity=""></path>
                        </svg>
                        <span class="absolute -translate-x-1/2 -translate-y-1/2 left-1/2 top-1/2">
                        <svg class="fill-error-600" width="38" height="38" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
