                ...
                @forelse($categories as $category)
                    <tr x-data="statusBadge">
                        <td class="px-6 py-3 whitespace-nowrap">
                            <div class="flex items-center">
                                <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ $category->title }}</p>
                            </div>
                        </td>
                        <td class="px-6 py-3 whitespace-nowrap text-center">
                            <span
                                x-text="{{ isset($category->active) }} ? getBadge({{ $category->active }}).text : getBadge().text"
                                :class="{{ isset($category->active) }} ? getBadge({{ $category->active }}).color : getBadge().color"
                        ...
                        <td class="px-6 py-3 whitespace-nowrap items-center">
                            <div class="flex w-full items-center justify-center gap-2">
                                ...
                                <x-common.confirm-delete
                                    ...
                                    route-name="{{ route('admin.categories.destroy', $category->id) }}">
                                    <!-- delete trash icon -->
                                    <button @click="itemId = 1" data-tippy-content="@lang('Delete Category')" class="flex items-center justify-center text-gray-500 hover:text-error-500 dark:text-gray-400 dark:hover:text-error-500">
                                        <svg class="fill-current" width="24" height="24" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
